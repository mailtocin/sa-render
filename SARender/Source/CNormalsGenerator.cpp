	...
	if (geometry->numMorphTargets > 0)
	{
		for (i = 0; i < geometry->numMorphTargets; i++)
		{
			...
			if (buffer.hasPositions)
			{
				...
				{
					if (!RwStreamReadReal(stream, geometry->morphTarget[i].verts, (UINT)geometry->numVertices * 12))
						return MORPHTARGETSLOADINGRESULT_ERROR;
					if (!RwStreamReadReal(stream, geometry->morphTarget[i].normals, (UINT)geometry->numVertices * 12))
						return MORPHTARGETSLOADINGRESULT_ERROR;
				}
				else
				{
